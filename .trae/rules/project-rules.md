...
2. **多端 = 微信官方 Donut + 云开发**：多端框架使用**微信官方 Donut**（小程序原生语法兼容），不使用 Taro / uni-app 等第三方跨端框架；后端用微信云开发（`wx.cloud`），不自建服务器。
3. **安全第一**：不硬编码密钥、envId 到代码里；云函数敏感操作一律在服务端校验权限，不信任前端传参。
4. **用户态 = 双轨制**：小程序端以 `_openid`（微信自动注入）为准；Donut App/H5 端以 `UNIONID` 或前端透传的可信身份标识为准，云函数入口统一使用 `cloudfunctions/_shared/identity.js` 的 `resolveIdentity()` 解析，禁止直接依赖单一 `OPENID`。
